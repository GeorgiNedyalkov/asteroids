...
int main()
{
    ...
    // Player vertices
    Vector2 v1, v2, v3;
    v1 = (Vector2){-25.0f, -25.0f};
    v2 = (Vector2){-25.0f, 25.0f};
    v3 = (Vector2){50.0f, 0.0f};
    ...
    while (!WindowShouldClose())
    {
		...
		// NOTE: Restrict to screen
        if (player.position.x < 0)
        {
            player.position.x = WINDOW_WIDTH;
        }
        else if (player.position.x > WINDOW_WIDTH)
        {
            player.position.x = 0;
        }
        ...
        else if (player.position.y > WINDOW_HEIGHT)
        {
            player.position.y = 0;
        }
        ...
        // NOTE :Clamp to screen
        // Teleport directly to other side directrly as a first version
        // Then wrap appropriately (using a Tourus math somehow)
        if (asteroid.position.x < 0)
        {
            asteroid.position.x = WINDOW_WIDTH;
        }
        else if (asteroid.position.x > WINDOW_WIDTH)
        {
        ...
        else if (asteroid.position.y > WINDOW_HEIGHT)
        {
        ...
        // 2. Rotate vertices
        Vector2 rot_1, rot_2, rot_3;
        rot_1.x = v1.x * cosf(radians(player.direction)) - v1.y * sinf(radians(player.direction));
        rot_1.y = v1.x * sinf(radians(player.direction)) + v1.y * cosf(radians(player.direction));

        rot_2.x = v2.x * cosf(radians(player.direction)) - v2.y * sinf(radians(player.direction));
        rot_2.y = v2.x * sinf(radians(player.direction)) + v2.y * cosf(radians(player.direction));

        rot_3.x = v3.x * cosf(radians(player.direction)) - v3.y * sinf(radians(player.direction));
        rot_3.y = v3.x * sinf(radians(player.direction)) + v3.y * cosf(radians(player.direction));
